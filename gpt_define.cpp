...
public:
    int x, y; // 物品坐标
    int appear_time;  // 物体出现时间 只停留20s(1000帧)

    Item(int _x, int _y, int _appera_time) : x(_x), y(_y), appear_time(_appera_time) {}
};
...
// 定义运输函数
int transport(Ship& ship, Berth& berth) {
    // 计算运输产生的价值
    int value = ship.capacity * berth.efficiency;
